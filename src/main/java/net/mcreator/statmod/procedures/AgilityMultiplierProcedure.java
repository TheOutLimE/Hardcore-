...
public class AgilityMultiplierProcedure {
	@SubscribeEvent
		...
		if (StatModModVariables.MapVariables.get(world).mobdammul > 0.5) {
			if (entity instanceof LivingEntity _livEnt ? _livEnt.getMobType() == MobType.UNDEAD : false) {
				if (entity instanceof LivingEntity _entity)
					_entity.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED, 20000000,
							(int) Math.floor(StatModModVariables.MapVariables.get(world).Agility), (true), (true)));
			} else {
